...
public class MemberRepository {
	...
	public Member load(Integer id) {
		String sql = "SELECT * FROM test_members WHERE id=:id";
	...
	public Member save(Member member) {
		...
		if (member == null) { // ←正しい条件に修正する
			...
			// ここに実行処理を書く
			member = template.update(insertSql, param);

		} else {
			String updateSql = "UPDATE test_members(name, age, dep_id) SET (:name, :age, :dep_id)";

			// ここに実行処理を書く
			member = template.update(updateSql, param);
		}
